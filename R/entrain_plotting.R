...
#' @param colorscale Plotly list colorscale object. 
...
plot_ligand_targets <- function(obj, 
    ...
    pseudotime_genes_covars_vector <- pseudotime_genes[["Covariances"]]
    names(pseudotime_genes_covars_vector) <- row.names(pseudotime_genes)
    top_pseudotime_genes <- pseudotime_genes_covars_vector %>% sort(., decreasing=TRUE) %>% head(n_top_targets)
...
plot_covarying_genes_scatter <- function(obj,
    ...
    pseudotime_genes_covars_vector <- pseudotime_genes[["Covariances"]]
    names(pseudotime_genes_covars_vector) <- row.names(pseudotime_genes)
    top_pseudotime_genes <- pseudotime_genes_covars_vector %>% sort(., decreasing=TRUE)
...
#' @param color_low Hex code of the color mapped to the lower limit. 
#' @param color_high Hex code of the color mapped to upper limit.
#' 
#' @return A ggplot2 plot object.
#' @export
plot_covarying_genes_heatmap <- function(obj,
    ...
    pseudotime_genes <- pathdata$pseudotime_associated_genes
    pseudotime_genes_covars <- pseudotime_genes[["Covariances"]]
    names(pseudotime_genes_covars) <- row.names(pseudotime_genes)
    pseudotime_genes_covars <- pseudotime_genes_covars %>% sort(., decreasing=TRUE)
    
    df <- pseudotime_genes_covars %>% head(n_top_genes) %>% as.data.frame()
    colnames(df) <- "Covariance"
    df$gene <- rownames(df)
...
#' @export
plot_ligand_trajectories <- function(obj, color_cells_by, n_top_ligands = 5,
...
plot_velocity_ligand_targets <- function(obj, 
    ...
    top_imps <- imps[,c("ligand", vcluster), drop=FALSE]
    top_imps <- top_imps[!is.na(top_imps[[vcluster]]),] %>% arrange(desc(get(vcluster))) %>% head(n_top_ligands)
    
    top_likelihds <- likelihds[,c("gene", vcluster), drop=FALSE]
    top_likelihds <- top_likelihds[!is.na(top_likelihds[[vcluster]]),] %>% arrange(desc(get(vcluster))) %>% head(n_top_targets)
    ...
    plot <- ggplot2::ggplot(df) +
        geom_tile(aes(x = gene,
                      ...
                      fill = potential)) +
        labs(y="Ligand", x = "Gene") + ggtitle(vcluster) +
        theme_minimal_big_font() 
...
velocity_ligand_importance_heatmap <- function(obj, 
    ...
    if (!is.null(vclusters)) {
        vcluster_imps <- lig_importances[,c("ligand", vclusters), drop=FALSE]
    } else {
...
#' @title Plot_velocity_likelihoods_heatmap
...
plot_velocity_likelihoods_heatmap <- function(obj, 
    ...
    if (!is.null(vclusters)) {
        vcluster_likelihds <- lik[,c("gene", vclusters), drop=FALSE]
    } else {
...
#' @param velocity_clusters Values in `adata.obs.velocity_cluster_key` denoting which velocity clusters and their ligands to visualize. 
#' @param velocity_cluster_key Column name of `adata.obs` metadata denoting velocity clusters.
